...
class PHPExcel
{
    ...
    {
        $fileTemplate = DATA_PATH . '/exportExcel/' . $template . '.' . $extension;
    ...
    public static function exportCompare(array $data, $fileName = 'export.xlsx', $template = 'default', $startCol = 2, $outputPath = '', $extension = 'xlsx', $format_columns = array(), $isSetBorder = false)
    {
        $fileTemplate = DATA_PATH . '/exportExcel/' . $template . '.' . $extension;
        $objReader = \PHPExcel_IOFactory::createReader($extension == 'xls' ? 'Excel5' : 'Excel2007');
        $objPHPExcel = $objReader->load($fileTemplate);
        if (!empty($data)) {
            ...
            foreach ($data as $colData) {
                self::fillCompare($objPHPExcel, $colData, $colIndex, $format_columns, $isSetBorder);
                $colIndex++;
            }
        }
        // auto set row height
    ...
    protected static function fillCompare($objPHPExcel, $colData, $colIndex, $format_columns, $isSetBorder){
        if (empty($colData) || empty($objPHPExcel)) {
            return;
        }
        $rowIndex = 3;
        $column = \PHPExcel_Cell::stringFromColumnIndex($colIndex);
        ...
        foreach ($colData as $key => $value) {
            if (array_key_exists($column, $format_columns) && $value !== '' && $value !== null) {
                ...
                if ($key == 'shortName') {
                    $objPHPExcel->getActiveSheet()->setCellValue(\PHPExcel_Cell::stringFromColumnIndex($colIndex) . $rowIndex, $value['value']);
                    $objPHPExcel->getActiveSheet()->getCell(\PHPExcel_Cell::stringFromColumnIndex($colIndex) . $rowIndex)->getHyperlink()->setUrl($value['url']);
                    $link_style_array = [
                        'font'  => [
                            'color' => ['rgb' => '0000FF'],
                            'underline' => 'single'
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle(\PHPExcel_Cell::stringFromColumnIndex($colIndex) . $rowIndex)->applyFromArray($link_style_array);
                } else {
                    $objPHPExcel->getActiveSheet()
                        ->getStyle(\PHPExcel_Cell::stringFromColumnIndex($colIndex) . $rowIndex)
                        ->getAlignment()
                        ->setWrapText(true);
                    $objPHPExcel->getActiveSheet()->setCellValueExplicit(\PHPExcel_Cell::stringFromColumnIndex($colIndex) . $rowIndex, $value);
                }
            }
            if ($isSetBorder) {
                $objPHPExcel->getActiveSheet()
            ...
            $rowIndex++; //Next Column will Be B, C, D....
        }
    }
